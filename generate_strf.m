...
function generate_strf(sessDate,areaName)

	% Load recording information data.
	load(fullfile('~','kiloSorted_DMR','Info','spike_data_info.mat'));
	if strcmp(areaName,'a1')
		myKsDir = dir_a1{find(contains(dir_a1(:,1),sessDate)),1};
	elseif strcmp(areaName,'belt')
		myKsDir = dir_belt{find(contains(dir_belt(:,1),sessDate)),1};
	else
	...
	trigger_loc = dir(fullfile('~','kiloSorted_DMR','Triggers','Cassius',sprintf('*%s*',sessDate)));
	trigger_file = trigger_loc.name;
	...
	%if nargin<5
	    %paramfile='C:\work\STRF\Moving_ripple\DMR_50HZ\DNR_Cortex_96k5min_4_50_param.mat';
	    paramfile = fullfile('~','kiloSorted_DMR','Stimulus','Moving_ripple','DMR_50HZ','DNR_Cortex_96k5min_4_50_param.mat');
	%end
	%if nargin<6
	    %sprfile='C:\work\STRF\Moving_ripple\DMR_50HZ\DNR_Cortex_96k5min_4_50.spr';
	    sprfile = fullfile('~','kiloSorted_DMR','Stimulus','Moving_ripple','DMR_50HZ','DNR_Cortex_96k5min_4_50.spr');
	%end
	fs=24414.0625;
	%sp = loadKSdir(myKsDir,areaName,blockName);
	sp = loadKSdir(myKsDir);
	%load(fullfile('I:\Parooa\Synapse\i\kiloSorted_DMR','Triggers','Cassius',trigger_file));
	load(fullfile('~','kiloSorted_DMR','Triggers','Cassius',trigger_file));
	warning('off','MATLAB:interp1:UsePCHIP')
	savepath = fullfile(myKsDir, 'STRFnp');
	s_bin=0.15;
	cs=1;
	...
	for s=1:length(index_clusters) 
	    ...
	    if ~isempty(STRF1)
	        ...
	        print(fullfile(savepath, ['STRF_Cluster' num2str(cluster_number)]),'-djpeg');
	        close all
	        conta=conta+1;
	    end
	
	end
	save(fullfile(savepath, 'strf.mat'),'STRFData','-v7.3');
