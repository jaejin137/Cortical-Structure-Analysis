		...
		for k = 1:numel(targetMatFiles)
			...
			for j = 1:nRecordings
				...
				for chn = 1:nChannel % data channels
					...
					CSDwave{chn,j,k} = z;
					CSDwavelet{chn,j,k} = [f cfs];

					%if allRecDep(j) == targetDepth && chn+1 == laminar_ac.LowerLamina3Sink(sessNum)
					%	if m==1
					%		tmplt_cassius(sessNum).LowerLamina3Sink = z;
					%	else
					%		tmplt_miyagi(sessNum).LowerLamina3Sink = z;
					%	end
					%	lineColor = 'b';
					%	lineWidth = 2;
					%elseif allRecDep(j) == targetDepth && chn+1 == laminar_ac.SupragranularSink(sessNum)
					%	if m==1
					%		tmplt_cassius(sessNum).SupragranularSink = z;
					%	else
					%		tmplt_miyagi(sessNum).SupragranularSink = z;
					%	end
					%	lineColor = 'c';
					%	lineWidth = 2;
					%elseif allRecDep(j) == targetDepth && chn+1 == laminar_ac.SupragranularSource(sessNum)
					%	if m==1
					%		tmplt_cassius(sessNum).SupragranularSource = z;
					%	else
					%		tmplt_miyagi(sessNum).SupragranularSource = z;
					%	end
					%	lineColor = 'm';
					%	lineWidth = 2;
					%else
