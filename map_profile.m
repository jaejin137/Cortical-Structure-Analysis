...
%for idx_target = [22 24 27 39 43]	% A1
%for idx_target = [9 12 14 19]		% lBelt
for idx_target = [19]
	try
		...
		%		openfig(fullfile(dir_resultFig,sprintf('Mr%s_%i',monkeyID,lamProf(idx_target).date),lamProf(idx_target).figName))
		%		openfig(fullfile(dir_resultFig,sprintf('Mr%s_%i',monkeyID,lamProf(k).date),lamProf(k).figName))
		...
		%%figOutDir = sprintf('AEP_MUA_CSD');
		%figOutDir = sprintf('PrelimData');
